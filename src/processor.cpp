  ...
  private:
      ...
      Context::ExceptionHandler&& eh)
    {
      if (get_call_depth() >= Consts::MAX_CALL_DEPTH) {
        eosio::check(false, "Reached max call depth (" + to_string(Consts::MAX_CALL_DEPTH) + ") (outOfBounds)");
      }
      ...
      const uint8_t pad = 0)
    {
      ...
      if (lastDst < offDst) {
        eosio::check(false, "Integer overflow in copy_mem (" + to_string(lastDst) + " < " + to_string(offDst) + ") (outOfBounds)");
      }

      if (lastDst > Consts::MAX_MEM_SIZE) {
        eosio::check(false, "Memory limit exceeded (" + to_string(lastDst) + " > " + to_string(Consts::MAX_MEM_SIZE) + ") (outOfBounds)");
      }
    ...
    void prepare_mem_access(const uint64_t offset, const uint64_t size)
    {
      ...
      if (end < offset) {
        eosio::check(false, "Integer overflow in memory access (" + to_string(end) + " < " + to_string(offset) + ") (outOfBounds)");
      }

      if (end > Consts::MAX_MEM_SIZE) {
        eosio::check(false, "Memory limit exceeded (" + to_string(end) + " > " + to_string(Consts::MAX_MEM_SIZE) + ") (outOfBounds)");
      }
    ...
    void jump_to(const uint64_t newPc)
    {
      if (ctxt->prog.jump_dests.find(newPc) == ctxt->prog.jump_dests.end()) {
        eosio::check(false, to_string(newPc) + " is not a jump destination (illegalInstruction)");
      }
      ctxt->set_pc(newPc);
    }
    ...
    void dispatch()
    {
      ...
      switch (op)
      {
        ...
        default:
          ...
          eosio::check(false, err.str() + " (illegalInstruction)");
      };
    }
    ...
    void call()
    {
      ...
      switch (op)
      {
        ...
        case Opcode::DELEGATECALL:
          ...
          break;
        default:
          eosio::check(false, "Unknown call opcode. (unexpectedState)");
      }
    }
  };
