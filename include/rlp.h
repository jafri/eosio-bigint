...
namespace eevm
{
  namespace rlp
  {
    ...
    struct from_bytes<size_t>
    {
      ...
      {
        if (size > 8) {
          eosio::check(false, "Trying to decode number: " + std::to_string(size) +
    ...
    struct from_bytes<std::array<uint8_t, N>>
    {
      std::array<uint8_t, N> operator()(const uint8_t*& data, size_t& size)
      {
        if (size != N)
        {
          eosio::check(false, "Trying to decode " + std::to_string(N) +
      ...
      const uint8_t*& data, size_t& size)
    {
      if (size == 0)
      {
        eosio::check(false, "Trying to decode length: got empty data");
      }
      ...
      if (length <= 0xbf)
      {
        ...
        {
          eosio::check(false, "Length of next element should be encoded in " +
      ...
      if (size < length_of_length)
      {
        eosio::check(false, "Length of next list should be encoded in " +
          std::to_string(length_of_length) + " bytes, but only " +
          std::to_string(size) + " remain");
      }
    ...
    std::tuple<Ts...> decode(const uint8_t*& data, size_t& size)
    {
      ...
      if constexpr (sizeof...(Ts) == 1 && !is_tuple<std::decay_t<Ts>...>::value)
      {
        ...
        {
          eosio::check(false, "Expected single item, but data encodes a list");
        }
      ...
      {
        eosio::check(false, "Expected list item, but data encodes a single item");
      }
      ...
      if constexpr (sizeof...(Ts) == 0)
      {
        ...
        {
          eosio::check(false, "Expected empty list, but data contains " +
