...
namespace eevm
{
  ...
  {
    if (size > 32) {
      eosio::check(false, "Calling from_big_endian with oversized array");
    }
    ...
    unsigned char* output)
  {
    ...
    rhash_keccak_update(&shactx, input, inputByteLen * std::numeric_limits<unsigned char>::digits);
    rhash_keccak_final(&shactx, output);
  }
