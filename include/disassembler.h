...
namespace eevm
{
  ...
  struct Instr
  {
    ...
    {
      if (!raw_imm.size()) {
        eosio::check(false, "Instruction does not have immediate.");
      }
  ...
  struct Disassembler
  {
    const static std::map<uint8_t, Op> ops;
    ...
    {
      Disassembly d;
      std::map<uint8_t, std::set<Instr*>> reverse_lookup;
